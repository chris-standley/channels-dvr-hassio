...
This app runs [Channels DVR Server](https://getchannels.com/dvr-server/) using
...
(Intel / AMD)** app — the only difference is the transcoding backend and the
host requirements below.

## ⚠️ Read this first: NVIDIA host requirements

NVIDIA GPU passthrough to a Home Assistant app is **not** officially
...
2. **A Home Assistant *Supervised* install** (or a self-managed Supervisor) on a
   ...
   drivers**, so this app cannot do NVENC on a stock HAOS device (Green,
   Yellow, ODROID, generic-x86-64 HAOS image, etc.).
3. **The `/dev/nvidia*` device nodes present on the host.** These are created by
   the driver. This app maps them in (see below). If your setup uses the
...
app instead, or run without hardware transcoding.
...
3. Start the app and open the web UI at `http://<your-ha-ip>:8089`.
...
> **Note:** this app uses a **separate** config directory from the Intel/AMD
> app (its slug differs). If you're migrating from the Intel/AMD app, copy
> the contents of the old app's config dir into this one, or point both at
...
binding to the host's port **8089**. Do not run this app and the Intel/AMD
app at the same time — they would both try to bind port 8089.

## Verifying NVENC works

- Check the app **Log** tab after starting a transcode; Channels logs the
...
Same two-track model as the Intel/AMD app: the **Channels server updates
...
- **App won't start / device error:** a mapped `/dev/nvidia*` node doesn't
