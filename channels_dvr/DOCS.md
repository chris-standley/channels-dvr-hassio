...
This app wraps the **official** `fancybits/channels-dvr` Docker image, so you
...
> `/addons/` (the `addon_configs`/local apps share) on your Home Assistant
> host and it will appear under **Local apps**.
...
app has no options to configure.

## Storage

| Purpose            | Container path  | Home Assistant location                     |
| ------------------ | --------------- | ------------------------------------------- |
| Config + database  | `/channels-dvr` | The app's own config dir (`addon_config`) |
| Recordings         | `/shares/DVR`   | The `/media` share                          |

- **Config/database** lives in the app's dedicated config directory and is
...
- **Recordings** are written to Home Assistant's **media** share, so they also
  show up when you browse `Media` and survive app reinstalls. Recordings can
  be very large — make sure the underlying storage has room.

> **Excluding recordings from backups:** Home Assistant backs up an app's
...
- **Drive 1 (HA data disk):** HA config, database, app configs — including
...
- **Drive 2 (media disk):** **Channels recordings** — via the `/media` share.
  ...
  drive 1. Any other app that mounts `/media` shares drive 2 too, so
  recordings sit alongside your other media (e.g. `/media/TV`, `/media/Movies`).
  See the repository README for the full walkthrough.

## Networking

The app runs with **host networking** (`host_network: true`). This is
...
This app maps `/dev/dri` into the container to enable **VA-API** hardware
...
Both are reached through the same `/dev/dri` render node, so this single app
...
- On boards **without** a `/dev/dri` render node (most ARM devices, including
  Home Assistant **Green** and **Yellow**), the app will fail to start with a
...
- For **NVIDIA** (NVENC) transcoding, install the separate **Channels DVR
  (NVIDIA)** app from this same repository. NVIDIA needs a different upstream
  image and driver setup — see that app's documentation.

## Time zone

Home Assistant Supervisor injects your configured time zone into the app
...
1. **The Channels DVR server updates itself.** The `fancybits/channels-dvr` image
   ...
   directory). Normal Channels updates need **no** app rebuild; you can also
...
2. **The Docker base image** (ffmpeg, OS libraries, GPU userspace) changes
   ...
   from the app's **Info** page, or use **Rebuild** (⋮ menu) to force a fresh
...
> during self-update — a Channels/image quirk, not this app. Restarting the
> app usually clears it.

## Troubleshooting

- **Tuners not found:** confirm the app is running with host networking and
